...
$not_login_arr =
array('login','logindl','act_login','act_logindl','register','register_xj','registerdlnew','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin','is_tel',
    'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered',
...
/* 显示页面的action列表 */
$ui_arr = array('register','register_xj', 'login','logindl', 'profile', 'registerdlnew','order_listdl','order_listfl','order_list', 'order_detail', 'address_list', 'collection_list',
...
if($action == 'chuange_mobile')
{
	...
	$pan = $db->getOne($sql);
	$res['qty'] = $pan;
	$sql = "SELECT  count(*) FROM ".$ecs->table('order_info')." WHERE  tel='".$mobile."' or mobile='".$mobile."' ";
	$pan1 = $db->getOne($sql);
	$res['qty1'] = $pan1;
    $sql2 = "SELECT  aite_id FROM ".$ecs->table('users')." WHERE office_phone='".$mobile."' or home_phone='".$mobile."' or mobile_phone='".$mobile."' ";
    $pan2 = $db->getOne($sql2);
    if(!empty($pan2)){
        $res['qty2'] = 1;
    }else{
...
if ($action == 'vip_mobile')
{
	...
	$res    = array('err_msg' => '', 'result' => '', 'qty' => 0);
	$mobile = $_REQUEST['mobile'];
	$sql = "SELECT  image_er,user_id FROM ".$ecs->table('users')." WHERE office_phone='".$mobile."' or home_phone='".$mobile."' or mobile_phone='".$mobile."' ";
	$pan = $db->getRow($sql);
...
elseif($action == 'is_tel'){//改写这个方法，webpos线上填的客户资料有手机号码，没账户密码等信息。需要关联会
    ...
    if($re){
		$sql = "select user_id,user_name from ".$GLOBALS['ecs']->table('users')." where  home_phone ='".$tel."' or mobile_phone='".$tel."'";
