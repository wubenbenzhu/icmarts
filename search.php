...
$facebook_code['value'] = str_replace('PageView', 'Search', $facebook_code['value']);
...
if ($_REQUEST['act'] == 'advanced_search')
{
    ...
    $smarty->assign('top_goods',       get_top5());           // 销售排行
...
elseif ($_REQUEST['act'] == 'page_list')
{
    ...
    $json   = new JSON;
    $jsonRes   = array('msgCode' => '', 'result' => '');
   // $_REQUEST = array_merge($_REQUEST, addslashes_deep($string));
    $_REQUEST['keywords']   = !empty($_REQUEST['keywords'])   ? htmlspecialchars(urldecode(trim($_REQUEST['keywords'])))     : '';
    ...
    {
        //long
        $sql = "select count(*) from ".$GLOBALS['ecs']->table('products')." where areaid = 0 and  product_sn_code = '".$_REQUEST['keywords']."'";
        if($GLOBALS['db']->getOne($sql)){
            $sql = "select goods_id from ".$GLOBALS['ecs']->table('products')." where areaid = 0 and product_sn_code = '".$_REQUEST['keywords']."'";
            ...
            $pro = implode(',', $pr);
            $product_code = 'and g.goods_id in ('.$pro.') ';
        }
        $sql = "select count(*) from ".$GLOBALS['ecs']->table('products')." where areaid = 0 and  product_sn = '".$_REQUEST['keywords']."'";
        
        if($GLOBALS['db']->getOne($sql)){
            $sql = "select goods_id from ".$GLOBALS['ecs']->table('products')." where areaid = 0 and product_sn = '".$_REQUEST['keywords']."'";
            $re = $GLOBALS['db']->getAll($sql);
            $pr=array();
            foreach($re as $k=>$v){
                $pr[]=$v['goods_id'];
    ...
    if(empty($product_code)){ //long
        $where = "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock ." ) ".$tag_where." )";
    }else{
        $where = $product_code;
    }
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in "." and g.area_shop_price like '%".$_SESSION['area_rate_id']."%' ".$where;
    ...
    {
        //$page = $max_page;
        $jsonRes["msgCode"] = "error";

        die($json->encode( $jsonRes));
    }

    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.online_sale_z,g.market_price,g.is_shipping, g.is_new, g.is_best, g.is_hot, g.online_sale, g.shop_price AS org_price, ".
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        $online_sale_arr = $row['online_sale'] ? explode(',', $row['online_sale'])  : '';

        if($online_sale_arr)
        {
            if (in_array($_SESSION['area_rate_id'], $online_sale_arr))
            {
                $arr[$row['goods_id']]['online_sale_display'] = 1;
            }
        }
        ...
        $sql = "SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods " .
                ...
                " AND g.goods_id ='".$row['goods_id'] ." and g.area_shop_price like '%".$_SESSION['area_rate_id']."%' ".
                "' AND (order_status = '" . OS_CONFIRMED . "' OR order_status = '" . OS_UNCONFIRMED . "' OR order_status = '5')";
        $rest = $GLOBALS['db']->getRow($sql);
        $arr[$row['goods_id']]['total_order'] = $rest['total_order'];
        if(empty($rest['total_goods']))
        {
            $rest['total_goods'] = 0;
        }
        $arr[$row['goods_id']]['total_goods'] = $rest['total_goods'];
    }
    ...
    if (!empty($arr))
    {
        $jsonRes['msgCode'] = 'success';
        $jsonRes['result'] = appendPage($arr);
...
else
{
    ...
    //判断搜索的是否条码 long
    if(!empty($_REQUEST['keywords'])){
        $sql = "select count(*) from ".$GLOBALS['ecs']->table('products')." where product_sn_code = '".$_REQUEST['keywords']."'";
        //是条码加入购物车
        if($GLOBALS['db']->getOne($sql)){
            $sql = "select * from ".$GLOBALS['ecs']->table('products')." where areaid = 0 and product_sn_code = '".$_REQUEST['keywords']."'";
            $rec = $GLOBALS['db']->getRow($sql);
            $attr = explode('|',$rec['goods_attr']);
            $_REQUEST['attr'] = $attr;


            /*$cart_res = addto_cart($rec['goods_id'],1,$attr);
    ...
    $max_page = ($count> 0) ? ceil($count / 5) : 1;
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price,g.is_shipping, g.online_sale_z, g.is_new, g.is_best, g.is_hot, g.online_sale, g.shop_price AS org_price, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type ".
            "FROM " .$ecs->table('goods'). " AS g ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
    ...
    $res = $db->SelectLimit($sql, 12, ($page - 1) * 12);
    ...
    /* 分页 */
    $url_format = "search.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" 
            . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
    ...
    if (isset($_REQUEST['pickout']))
    {
      ...
      $url_num = $url_format."&amp;min_price=" . $_REQUEST['min_price'] ."&amp;max_price=" . $_REQUEST['max_price'] . "&amp;sort=shop_price"."$attr_url&amp;order=$order&amp;page=";
    $url_zh = $url_format."&amp;min_price=" . $_REQUEST['min_price'] ."&amp;max_price=" . $_REQUEST['max_price'] . "&amp;sort=goods_id"."$attr_url&amp;order=$order&amp;page=";
    $url_date = $url_format."&amp;min_price=" . $_REQUEST['min_price'] ."&amp;max_price=" . $_REQUEST['max_price'] . "&amp;sort=last_update"."$attr_url&amp;order=$order&amp;page=";
    ...
    $pager = get_pager('search.php', $pager['search'], $count, $page, 5);
    $pager['display'] = $display;

    $smarty->assign('url_date',$url_date);
    $smarty->assign('url_zh',$url_zh);
    $smarty->assign('url_num',$url_num);
    $smarty->assign('url_format', $url_format);
    $smarty->assign('pager', $pager);

    /* 将参数合成url字串 */
    $param_url = 'act=page_list&';
   
    foreach ($pager['search'] AS $key => $value)
    {
        $param_url .= $key . '=' . $value . '&';
    }

    $smarty->assign('urlParam',$param_url);
...
function appendPage($res){
    ...
    $result = '';
    foreach ($res as $k => $v) {
        $result .= '<div class="box-w25">';
        if($v['online_sale_z']==1)
        {
            $result .= '<div class="item online">';
        }
        else
        {
            $result .= '<div class="item">';  
        }
        $result .='<div class="mobile-shadow"></div>';
