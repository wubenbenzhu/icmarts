...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('goods_activity')." WHERE act_type = '1' AND act_id = '$group_buy_id' and areaid like '%".$_SESSION['area_rate_id']."%'";
    $co = $db->getOne($sql);
    if($co <= 0){
        ecs_header("Location: ./\n");
        exit;
...
//    }
    ...
    if (!$smarty->is_cached('group_buy_goods.dwt', $cache_id))
    {
       ...
       $listgroup =  group_buy_list(3,1," and b.act_id != '$group_buy_id' ");
		$smarty->assign('listgroup',$listgroup);
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    if ($specs)
    {
        ...
        $product_info = get_products_info($goods['goods_id'], $_specs,$_SESSION['yuming']);
    }
    ...
    if(!$specs){
        $sql="select product_id, product_number from ".$GLOBALS['ecs']->table('products')." where goods_id = ".$goods['goods_id']." and areaid = ".$_SESSION['yuming'];
        $product = $GLOBALS['db']->getRow($sql);
        if($number>$product['product_number']){
           // show_message($_LANG['gb_error_goods_lacking'], '', '', 'error');
        }
 ...
 * @return  array
 */
function group_buy_list($size, $page,$whereqi='')
{
    /* 取得团购活动 */
    $gb_list = array();
    $now = gmtime();
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, ".
            ...
            "WHERE b.act_type = '" . GAT_GROUP_BUY . "' " .$whereqi.
            "AND b.start_time <= '$now' AND end_time >= '$now' AND b.is_finished < 3 and areaid like '%".$_SESSION['area_rate_id']."%' ";
    $sql.=" ORDER BY b.act_id DESC";
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $group_buy['shop_price'] = $goods['shop_price_formated'];
        /* 格式化时间 */
        ...
        $sql = "SELECT price FROM ".$GLOBALS['ecs']->table('price_area') ." WHERE hd_id=".$group_buy['act_id']." AND price_type='group_price_deposit' "." AND goods_id='".$group_buy['goods_id']."'" ." and areaid=0 AND  areaid_rate='".$_SESSION['area_rate_id']."'";
        $group_buy['deposit'] = $GLOBALS['db']->getOne($sql);
        /* 格式化保证金 */
        ...
        else
        {
            ...
            {
            	$sql = "SELECT price FROM ".$GLOBALS['ecs']->table('price_area') ." WHERE num=".$amount_price['amount']." and hd_id=".$group_buy['act_id']." AND price_type='group_price' "." AND goods_id='".$group_buy['goods_id']."'" ." and areaid=0 AND  areaid_rate='".$_SESSION['area_rate_id']."'";
            	$amount_price['price'] = $GLOBALS['db']->getOne($sql);
            	$price_ladder[$key]['price'] = $amount_price['price'];
                $price_ladder[$key]['formated_price'] = price_format($amount_price['price']);
            }
        }
        $current_num =0;
        $cur_price  = $price_ladder[0]['price']; // 初始化
        $stat = group_buy_stat($group_buy['act_id'], $group_buy['deposit']);
