...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    //updata_cart_vo();
	$arelist = $db->getAll("SELECT areaid,areaname FROM ".$ecs->table('area')." WHERE AreaExchangeRateId='".$_SESSION['area_rate_id']."'   AND state=1  ");
    ...
    if(!empty($region[2]))
    {
    $sql=" select region_name from ".$ecs->table('region')." where region_id = ".$region[2];
    $r_name = $db->getOne($sql);
    $sql="select * from ".$ecs->table('shipping_sf')." where region_id = ".$region[1]." and parent_id = ".$region[0]." and three_id = '".$region[2]."'";
    $sf_list['list'] = $db->getAll($sql);
        $sql="select * from ".$ecs->table('shipping_sf')." where region_id = ".$region[1]." and parent_id = ".$region[0]." and three_id = '".$region[2]."' and sf_name in('順豐站','速遞寶智能自取櫃','服務中心')";
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   $order['dlfcmoney_hk'] = $total['dlfc_surplus_hk'];
    ...
    /* 给商家发邮件 */
    /* 增加是否给客服发送邮件选项 */
    if ($_CFG['send_service_email'] && $_CFG['service_email'] != ''&& 1<>1)//邮件后台设置有问题，发邮件时间太多。暂时不进发邮件功能
    {
        ...
        send_mail($order['consignee'], trim($order['email']), '訂單確認', '親愛的'.$order['consignee'].'，妳好！ 我們已經收到您於 '.$order['formated_add_time'].' 提交的訂單，該訂單編號為：'.$order['order_sn'].' 請記住這個編號以便日後的查詢。'.$_CFG['shop_name'].date($_CFG['time_format']), $tpl['is_html']);
...
//new wuxurong
    ...
    $smarty->assign('fuhe',p_activity_son());
  	/*$sql = "SELECT * FROM " . $ecs->table('cart_activity') ."where recs_id=115";
    $test = $db->getRow($sql);
    $testgoods = unserialize($test['buy_goods_id']);x
    var_dump($testgoods);die();*/
	//star 查询购物车是否有买几活动 
    ...
    while ($row = $db->fetchRow($res)) {
    	...
    	$sql = " SELECT sum(song-ysong) FROM ".$ecs->table('cart_activity')." WHERE is_finish=0 and song>ysong  and session_id = '" .SESS_ID. "' and act_id = ".$row['act_id'];
    	...
    	$sql = "SELECT sum(goods_number-song_num) FROM ".$ecs->table('cart')." WHERE session_id = '" .SESS_ID. "'  and dl_dlsp <>1 AND goods_id in(".$row['act_range_ext'].")".
    			" AND parent_id = 0 " .
    			" AND extension_code not like '%package_buy%' " .
    			" AND rec_type = 'CART_GENERAL_GOODS' AND zengsong=0 and activity_id=0 AND goods_number>song_num ";
    	
    	$number_buy = $db->getOne($sql);
    	$row['buy_number'] = $number_buy;
    	...
    	if($pan_num == 1)
    	{
    		$goods_pan = explode(',', $row['act_range_ext']);
    		$goods_cun = array();
    		$i = 0;
    		foreach ($goods_pan as $gey=>$galue)
    		{
    			foreach ($cart_goods_list as $pey=>$palue)
    			{
    				if($galue == $palue['goods_id'])
    				{
    					if($palue['goods_number']>$palue['song_num']&&$palue['zengsong']==0)
    					{
	    					$goods_cun[$i] = $cart_goods_list[$pey];
	    					$goods_cun[$i]['canyushu']  = $goods_cun[$i]['goods_number'] - $goods_cun[$i]['song_num'];
	    					$i = $i+1;
    					}
    				}
    			}
    		}
    		$row['goods_list'] = $goods_cun;
    		$pan_song = 0;
    		//优惠方式
    		...
    		if (!empty($row['gift'])) {
    			...
    			foreach ($row['gift'] as $k => $v) {
    				...
    				$row['gift'][$k]['specification'] =   $spe;
    			}
    		} else {
    			$row['gift'] = '';
    		}
    		$row['pan_song'] = $pan_song;// 用户判断是否设置有赠送商品
    		$favourable_lists[$j] = $row;
    		$j=$j+1;
...
function update_favourable_activity_price()
{
	...
	$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' and extension_code='package_buy_all' and cf_huodong=0 ";
	$GLOBALS['db']->query($sql);
    $sql = "select * from ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' and activity_id>0  and cf_huodong=0 ";
    ...
    foreach ($cart_list as $key=>$value)
    {
        ...
        $sql = "update ".$GLOBALS['ecs']->table('cart')." set zengsong=0,activity_id=0,song_buy=0,prec_id=0,extension_code='',carta_id=0,is_jianshu=0,song_num=0,goods_price=".$goods_price.
            " where rec_id = ".$value['rec_id'];
        $GLOBALS['db']->query($sql);
    }

	$sql = "select * from ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' and activity_id=0 and extension_code='' and goods_price>0  and cf_huodong=0 ";
    ...
    $sql = "select rec_id, sum(goods_number) as goods_num,count(*) as num,goods_id,goods_attr_id from ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' and activity_id=0 and extension_code='' and cf_huodong=0 group by goods_id,goods_attr_id";
    ...
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('favourable_activity') .
        " where act_type >2 and user_rank in(".$_SESSION['user_rank'].") and  act_range_ext <> '' and ".gmtime()." >= start_time and ".gmtime()." <= end_time ORDER BY sort_order ASC,end_time DESC";

    $res = $GLOBALS['db']->query($sql);
    $sql = " SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id ='".SESS_ID."' and cf_huodong = 0";//查看是否有参与重复活动的商品
            ...
            if($value['goods_id']==$v)
            {
                foreach ($cun_hd as $kk=>$vv) //过滤掉重复的商品集合
                {
                    ...
                    foreach ($gl_list as $kkk=>$vvv)
                    {
                        ...
                        {
                            $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET  cf_huodong = 1 WHERE goods_id in(".$vv.") and session_id ='".SESS_ID."'";
                            $GLOBALS['db']->query($sql);
                        }
                    }
                }
	...
	if($hy_cf==0)//购物车不含有参加重复活动的商品
	{
		...
		while ($row = $GLOBALS['db']->fetchRow($res)) { //循环所有活动，把商品加入合适的活动里
			...
			$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '" .SESS_ID. "'  and dl_dlsp <>1 AND goods_id in(".$row['act_range_ext'].")".
					" AND parent_id = 0 " .
					" AND extension_code not like '%package_buy%' " .
					" AND rec_type = 'CART_GENERAL_GOODS' AND zengsong=0 and cf_huodong=0  AND goods_number>=song_num  order by goods_id";
                ...
                elseif($row['act_type'] == 3)//買几送几設置
                {
                    ...
                    if(empty($aract)) { //没设有赠品
                        $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '" .SESS_ID. "'  and dl_dlsp <>1 AND goods_id in(".$row['act_range_ext'].")".
                            " AND parent_id = 0 " .
                            " AND extension_code not like '%package_buy%' " .
                            " AND rec_type = 'CART_GENERAL_GOODS' AND zengsong=0 and cf_huodong=0  AND goods_number>=song_num  order by goods_price desc";
    ...
    $sql = "select * from ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' and activity_id=0 ";
    $goods_ac=$GLOBALS['db']->getAll($sql);

    $sql = "select * from ".$GLOBALS['ecs']->table('cart')."WHERE session_id = '".SESS_ID."' and activity_id>0 group by activity_id";
...
$loginUrl = $helper->getLoginUrl('http://www.icmarts.com/user1.php?act=fboath_login&type=facebook', $permissions);
return $loginUrl;
}*/
?>