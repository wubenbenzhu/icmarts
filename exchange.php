...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('exchange.dwt', $cache_id))
    {
        ...
        $smarty->assign('hot_goods',       get_exchange_recommend_goods('hot',  $children, $integral_min, $integral_max,'',$_SESSION['yuming']));


        $count = get_exchange_goods_count($children, $integral_min, $integral_max,'',$_SESSION['yuming']);
        $max_page = ($count> 0) ? ceil($count / $size) : 1;
        if ($page > $max_page)
        {
            $page = $max_page;
        }
        $goodslist = exchange_get_goods($children, $integral_min, $integral_max, $ext, $size, $page, $sort, $order,$_SESSION['yuming']);
        if($display == 'grid')
        {
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        /* 获得商品的信息 */
        $goods = get_exchange_goods_info($goods_id,$_SESSION['yuming']);
        ...
        else
        {
            ...
            if($_SESSION['yuming'] !=0)
            {
            	$dianpuwhere = " AND eg.areaid like '%".$_SESSION['yuming']."%' ";
            }
            
            /* 上一个商品下一个商品 */
            $sql = "SELECT eg.goods_id FROM " .$ecs->table('exchange_goods'). " AS eg," . $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id > " . $goods['goods_id'] .$dianpuwhere.
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime(). " AND eg.is_exchange = 1 AND g.is_delete = 0 LIMIT 1";
            ...
            $sql = "SELECT max(eg.goods_id) FROM " . $ecs->table('exchange_goods') . " AS eg," . $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < ".$goods['goods_id'] . $dianpuwhere.
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime()." AND eg.is_exchange = 1 AND g.is_delete = 0";
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    if (!empty($specs))
    {
        ...
        $product_info = get_products_info($goods_id, $_specs,$_SESSION['yuming']);
    }
 ...
 * @return  array
 */
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order,$dianpu='')
{
    ...
    if($dianpu !=0)
    {
    	$dianpuwhere = " AND eg.areaid like '%".$dianpu."%' ";
    }
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            "WHERE eg.goods_id = g.goods_id AND $where $dianpuwhere".
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime()." $ext ORDER BY $sort $order";
 ...
 * @return  integer
 */
function get_exchange_goods_count($children, $min = 0, $max = 0, $ext='',$dianpu='')
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' .
           $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND $where $ext".$dianpuwhere.
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime();
 ...
 * @return  array
 */
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='',$dianpu='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'WHERE eg.is_exchange = 1 AND g.is_delete = 0 ' .$dianpuwhere.
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime(). $price_where . $ext;
    $num = 0;
 ...
 * @return  void
 */
function get_exchange_goods_info($goods_id,$dianpu="")
{
	if($dianpu !=0)
	{
		$dianpuwhere = " AND eg.areaid like '%".$dianpu."%' ";
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, eg.exchange_integral, eg.is_exchange ' .
            ...
            "WHERE g.goods_id = '$goods_id' AND g.is_delete = 0 ".$dianpuwhere .
            " AND eg.start_time <= " . gmtime() .
            " AND eg.end_time >= " . gmtime().
            ' GROUP BY g.goods_id';
